window.onload = () => {
    function displayCart() {
        ...
        cart.forEach((product, i) => {
            ...
            html += `<tr>
              ...
              <td id="actions-primary-${product.id}">
                  <button data-id="${product.id}" class="btn btn-primary cart-item-update" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Izmeni</button>
                  <button id="${product.id}" class="btn btn-primary cart-item" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Obriši</button>
              </td>
              <td id="actions-secondary-${product.id}" style="display: none">
                  <button data-id="${product.id}" class="btn btn-primary cart-item-save" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Sačuvaj</button>
                  <button data-id="${product.id}" class="btn btn-primary cart-item-cancel" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Otkaži</button>
              </td>
            </tr>`
        })
        ...
        $('.cart-item-update').each(function () {
            $(this).on('click', function () {
                ...
                const html = `<input type="number" id="count-input-${id}" min="1" class="form-control" id="exampleFormControlInput1" value="${product.count}">`
                $(`#count-${id}`).html(html);
    ...
    $('#finish-order-btn').on('click', function () {
        ...
        if (!regexImePrezime.test(fullName) || !fullName.length) {
            ...
            $('#fullNameControlErr').html('Ime i prezime moraju početi velikim slovom.');
        } else {
        ...
        if (!phoneRegex.test(phone) || !phone.length) {
            ...
            $('#contactControlErr').html('Neispravno ste uneli broj telefona.');
        } else {
