window.onload = () => {
    ...
    fetchData("levels", displayLevels);
    ...
    function returnLevel(ids) {
        const level = levels.find(x => ids === x.id);
        return level ? level.value : '';
    }

    function displayLevels(data) {

        htmlCheckBox(data, "filterLevels", "#levels", 'level');
        levels = data;
        $("input[name=filterLevels]").change(filterChange)
    ...
    function displayProducts(data) {
        ...
        setCheckBoxCount(levels, data, "filterLevels", 'level');
        let html = "";
            ...
            html += `<div class="col-6 col-lg-3 pb-3">
                <div class="card product-card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text"></p>
                        <span class="price" style="color: black">${el.price.newPrice}<small class="h6">RSD</small> </span>
                        <s class="text-danger">${el.price.oldPrice}<small class="h6">RSD</small></s>
                    </div>
                    <div class="card-body col">
                        <div class="d-grid gap-2 mt-2">
                            <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal${el.id}" class="btn btn-primary" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Pogledaj</button>
                            <button id="${el.id}" class="btn btn-primary card-link add-cart-btn-1" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Kupi</button>
                        </div>
                    </div>
                    <div class="card-body col" id="add-to-cart-${el.id}" style="display: none">
                        ...
                        <input type="number" id="count-${el.id}" min="1" class="form-control" id="exampleFormControlInput1" value="${count}">
                        <div class="d-grid gap-2 mt-2">
                          <button data-id="${el.id}" class="btn btn-primary add-cart-btn" type="button" style="background-color: rgb(230, 122, 55); border-color: rgb(230, 122, 55)">Dodaj u korpu</button>
                        </div>
                    </div>
                </div>
            </div>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <img src="${el.img.src}" alt="${el.img.alt}"
                    ...
                    <p class="card-title">Naprednost: <h4>${returnLevel(el.level)}</h4></p>
                    <span>Cena:</span>
                    <p>
        ...
        $("#products").html(html || 'Trenutno nema rezultata. Sadržaj je u pripremi.');
    ...
    function levelFilter(data) {
        let selectedItems = [];
        $('input[name=filterLevels]:checked').each(function (el) {
            selectedItems.push(parseInt($(this).val()));
        });
        if (selectedItems.length != 0) {

            return data.filter(x => selectedItems.includes(x.level));
