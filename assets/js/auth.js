...
const register = event => {
    ...
    if (!regexImePrezime.test(nameSurnameVal) || nameSurnameVal.length == 0) {
        ...
        $('#nameLastNameErr').html('Ime i prezime moraju početi velikim slovom.')
    ...
    if (!regexMail.test(emailVal) || emailVal.length == 0) {
        ...
        $('#emailErr').html('E-mail adresa mora biti u formatu username@example.com');
    ...
    if (!regPassword.test(passwordVal) || passwordVal.length == 0) {
        ...
        $('#passwordErr').html('Lozinka mora imati barem 8 karaktera, od toga jedan broj i jedno slovo.')
    } else {
